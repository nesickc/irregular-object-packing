...
| --- | --- | --- | --- | --- | --- | --- |
| IROP-COMPAT-0001 | Preserved | The current scale barrier is passed as `max_scale`, but the local NLP receives no finite upper scale bound. The returned scale is multiplied into the previous scale and clamped after solving. | Preserve the effective solve-then-clamp behavior for parity, behind a named compatibility path, then independently validate the clamped geometry before acceptance. | Enforcing the barrier inside the solve can change optimized rotations and translations, so downstream impact is plausible. The postcheck prevents a clamped infeasible result from corrupting state. | Milestone 4 | Verified |
...
| IROP-COMPAT-0006 | Preserved | The local NLP composes its incremental rotation matrix with the already transformed vertex geometry, but result application adds the three returned Euler components to the stored angles. General Euler addition is not the same matrix composition. | Preserve componentwise Euler addition for parity and independently validate the geometry produced by the actually stored transform before publishing it. | Changing the update representation can materially alter every subsequent CAT rebuild, collision result, and placement. The independent postcheck preserves successful behavior without accepting the representation mismatch when it is infeasible. | Milestone 4 | Verified |
